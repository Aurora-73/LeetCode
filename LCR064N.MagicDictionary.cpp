...
实现 MagicDictionary 类：
	...
	void buildDict(String{}dictionary) 使用字符串数组dictionary 设定该数据结构，
...
public:
	...
	bool search(const string &searchWord) {
		return help_search(searchWord, 0, root, 0);
	}
...
private:
	...
	bool help_search(const string &searchWord, int i, TireNode *p, bool changed) {
		...
		bool res = help_search(searchWord, i + 1, real, changed);
		if(!res && !changed) {
			for(int j = 0; j < 26 && !res; ++j) {
				if(p->childs[j] != real) {
					res = help_search(searchWord, i + 1, p->childs[j], 1);
				}
			}
		}
		return res;
