...
}; // 先用全部word构建Trie树，并将相同长度的word放到一起作为一系列可选的行分组，在每一个行分组上每次选择下一个行，并且移动每一列的TrieTree节点的位置
// 超时，主要有两个优化点：1、在Trie的路径上记录最大长度，如果当前的最大长度*宽度小于已保存的最大值，则没有必要往下继续
//                       2、将指针型TrieTrie修改为vector型
